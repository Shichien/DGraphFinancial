...
# --- (新增) 引入 Optuna 和 lightgbm (用于 early_stopping) ---
try:
    import optuna
    import lightgbm as lgb
except ImportError:
    print("=" * 50)
    print("!! 缺少 'optuna' 或 'lightgbm' 库 !!")
    print("请先运行: pip install optuna lightgbm")
    print("=" * 50)
    exit()
...
data_ones = np.ones(E, dtype=np.float32)  # 重命名以避免与 'data = np.load' 冲突
...
# -----------------------------------------------------------------
# --- 🚀 (新增) “关系感知”特征工程 (灵感来源: RA-GNN 论文) ---
# -----------------------------------------------------------------
# 我们不再“混”在一起计算邻居特征
# 而是为每一种 edge_type 单独计算邻居特征

print("--- Starting Relation-Aware Feature Engineering (RA-GNN inspired) ---")

relation_aware_feats_list = []
unique_types = np.unique(edge_type)
print(f"Found {len(unique_types)} unique edge types.")

# 缓存所有节点的 deg, in_deg, out_deg, max_day_node
# (确保它们在循环外已经计算好，你的代码已经做到了)

for r_type in unique_types:

    # 1. 筛选出当前类型的边
    type_mask = (edge_type == r_type)
    r_edge_index = edge_index[type_mask]
    r_E = r_edge_index.shape[0]

    if r_E == 0:
        # 如果这种类型的边不存在，跳过
        continue

    print(f"  > Processing relation type {r_type} ({r_E} edges)...")

    # 2. 构建特定类型的邻接矩阵 A_in_r, A_out_r
    r_rows_in, r_cols_in = r_edge_index[:, 1], r_edge_index[:, 0]
    r_rows_out, r_cols_out = r_edge_index[:, 0], r_edge_index[:, 1]
    r_data = np.ones(r_E, dtype=np.float32)

    A_in_r = coo_matrix((r_data, (r_rows_in, r_cols_in)), shape=(N, N))
    A_out_r = coo_matrix((r_data, (r_rows_out, r_cols_out)), shape=(N, N))
    A_all_r = ((A_in_r + A_out_r) > 0).astype(np.float32)

    # 3. 计算“分类型”的邻居统计特征 (就像你之前做的那样)

    # 度
    r_num_in_neighbors = np.array(A_in_r.sum(axis=1)).flatten()
    r_num_out_neighbors = np.array(A_out_r.sum(axis=1)).flatten()

    # 邻居的平均度数 (使用全局的 deg, in_deg, out_deg)
    r_mean_in_deg_neighbors = A_in_r.dot(in_deg) / np.maximum(r_num_in_neighbors, 1)
    r_mean_out_deg_neighbors = A_out_r.dot(out_deg) / np.maximum(r_num_out_neighbors, 1)
    r_mean_neighbor_deg = A_all_r.dot(deg) / np.maximum(A_all_r.sum(axis=1).A1, 1)

    # 邻居的平均最后活跃时间 (使用全局的 max_day_node)
    r_mean_last_active_in = A_in_r.dot(max_day_node) / np.maximum(r_num_in_neighbors, 1)
    r_mean_last_active_out = A_out_r.dot(max_day_node) / np.maximum(r_num_out_neighbors, 1)

    r_feats = np.stack([
        r_num_in_neighbors,
        r_num_out_neighbors,
        r_mean_in_deg_neighbors,
        r_mean_out_deg_neighbors,
        r_mean_neighbor_deg,
        r_mean_last_active_in,
        r_mean_last_active_out,
    ], axis=1)

    relation_aware_feats_list.append(r_feats)

# 4. 拼接所有“分类型”的特征
if relation_aware_feats_list:
    relation_aware_feats = np.concatenate(relation_aware_feats_list, axis=1).astype(np.float32)
    print("relation_aware_feats shape:", relation_aware_feats.shape)

    # 5. 将新特征加入到 struct_feats 中
    struct_feats = np.concatenate([struct_feats, relation_aware_feats], axis=1)
    print("Updated struct_feats with Relation-Aware features:", struct_feats.shape)
else:
    print("No relation-aware features were generated.")
...
def objective(trial):
    ...
    params = {
        ...
        'subsample': trial.suggest_float('subsample', 0.6, 1.0),  # 随机采样行
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.6, 1.0),  # 随机采样特征
        'reg_alpha': trial.suggest_float('reg_alpha', 0.0, 1.0),  # L1 正则化
        'reg_lambda': trial.suggest_float('reg_lambda', 0.0, 1.0),  # L2 正则化
    }

    # --- B. 训练模型 ---
    # 注意：这里我们使用本地的 train/val 进行调优
    clf = LGBMClassifier(**params)

    # 使用 early_stopping，如果 50 轮验证集没提升，就停止
    ...
    return val_auc  # Optuna 会自动最大化这个返回值


# 创建一个“研究”，方向是“最大化”
study = optuna.create_study(direction='maximize')
# 开始优化！ n_trials=50 表示它会尝试 50 组不同的参数组合
...
# 保存最佳参数，以便在 Bagging 循环中使用
best_params_lgbm = study.best_params

# -----------------------------------------------------------------
# --- (结束) Optuna 调优部分 ---
# --- 接下来是原始的 Bagging 训练，但会使用上面的 best_params_lgbm
# -----------------------------------------------------------------
...
models = [
    ...
    {"name": "LightGBM", "cls": LGBMClassifier},  # 占位符
]
