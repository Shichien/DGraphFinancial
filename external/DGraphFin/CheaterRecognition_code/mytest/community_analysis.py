...
try:
    ...
    print("=" * 50)
    print("!! 缺少 'networkx' 或 'python-louvain' 库 !!")
    print("请先运行: pip install networkx python-louvain")
    print("=" * 50)
    exit()
...
edge_feats_final_list = [
    ...
    std_in_deg_neighbors.reshape(-1, 1), std_out_deg_neighbors.reshape(-1, 1),
    std_neighbor_deg.reshape(-1, 1), std_last_active_in.reshape(-1, 1),
...
# -----------------------------------------------------------------
# --- (🚀 新增) 步骤 6：社区发现 (Community Detection) ---
# -----------------------------------------------------------------
print("\n--- Starting Community Detection (Louvain) ---")
print("Building graph for NetworkX...")
# (!!! 警告: 这一步在 400w 节点上会非常慢 !!!)
# (我们使用无向图 G，这对于社区发现是标准的)
G = nx.Graph()
G.add_nodes_from(range(N))
G.add_edges_from(edge_index)  # networkx 会自动去重

print("Running Louvain... (This may take hours)")
# 这是一个字典 {node_id: community_id}
partition = community_louvain.best_partition(G)
print("Louvain calculation complete.")

community_id_feat = np.array([partition.get(i, -1) for i in range(N)]).astype(np.int32)

all_feature_arrays.append(community_id_feat.reshape(-1, 1))
all_feature_names.append('community_id')  # 1 个
print(f"Added 1 new 'community_id' feature.")
# 总特征数 = 118 + 1 = 119
...
# (!!! 关键) 这是你上次运行 find_features.py 找出的 25 个“琐碎特征”
# (注意：'community_id' 是新特征，不会在被删除的列表里)
TRIVIAL_FEATURES = [
...
# (!!!) 也从名称列表中删除，这对 'categorical_feature' 至关重要
final_feature_names = all_feature_names.copy()
for index in useless_indices:
    final_feature_names.pop(index)

# 119 - 25 = 94
print(f"Final features after removal: {X_final.shape[1]} (Names: {len(final_feature_names)})")
if 'community_id' not in final_feature_names:
    print("警告：'community_id' 特征被错误地移除了！")

# -----------------------------------------------------------------
# --- 步骤 8：准备训练 (逻辑不变，但使用 X_final) ---
# -----------------------------------------------------------------

X = X_final  # (!!!) 使用我们精炼后的 94 维 X
D = X.shape[1]
...
# (!!!) 准备传递给 fit 的特征名和类别特征
optuna_feature_names = final_feature_names.copy()
optuna_categorical_feature = ['community_id']

# -----------------------------------------------------------------
# --- 步骤 10：Optuna 自动调参 (在精炼+社区特征上) ---
# -----------------------------------------------------------------
...
def objective(trial):
    ...
    clf.fit(X_train_optuna, y_train_optuna,
            ...
            # (!!! 关键) 告诉 Optuna 哪个是类别特征
            feature_name=optuna_feature_names,
            categorical_feature=optuna_categorical_feature
            )
...
# (!!!) 准备传递给 fit 的特征名和类别特征
train_feature_names = final_feature_names.copy()
train_categorical_feature = ['community_id']
    ...
    for i, train_idx_chunk in enumerate(chunks):
        ...
        if name == "LightGBM":
            ...
            clf.fit(X_train_chunk, y_train_chunk,
                    ...
                    # (!!! 关键) 告诉 LGBM 哪个是类别特征
                    feature_name=train_feature_names,
                    categorical_feature=train_categorical_feature
                    )
        else:
            # (!!!) HistGB 也支持类别特征，但 XGBoost 不直接支持
            # 为了简单，我们只给 LGBM
...
OUTPUT_NPY = "submission_refined_community.npy"  # 改个名
